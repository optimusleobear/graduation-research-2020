...
class GUIDraw(QWidget):
    ...
    def __init__(self, model, os_type, gpu, dist_model=None, load_size=256, win_size=512):
    ...
    def save_result(self):
        ...
        true_file_name = path.rsplit('/', 1)[1].split('.')[0]
        save_file_name = true_file_name + '_color.png'
        ...
        cv2.imwrite(os.path.join(save_path, save_file_name), self.model.get_img_fullres()[:, :, ::-1])


    def save_result_x(self):
        path = os.path.abspath(self.image_file)
        path, ext = os.path.splitext(path)
        print(path)
        ...
        saved_file = save_path + '/' + save_file_name

        realsr.realsr(saved_file, self.os_type, self.gpu)
